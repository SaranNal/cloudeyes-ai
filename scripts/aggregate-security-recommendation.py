    ...
    for customer in customers: 
        ...
        for account_data in security_recommendation_data.find():
            ...
            if "security" in recommendation:
                security_recommendation = [d['name'] for d in recommendation['security']]
                new_formatted_data['$set']['security_recommendation'] = security_recommendation
            if "cost_optimizing" in recommendation:
                cost_optimizing = [d['name'] for d in recommendation['cost_optimizing']]
            ...
            result = agg_security_recommendation_data.update_one(filter_criteria, new_formatted_data, upsert=True)
            print(f"Matched {result.modified_count} document(s) and updated {result.modified_count} document(s).")
